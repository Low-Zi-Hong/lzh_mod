package lzh.net.feature.renderingSystem;
...
public class blockRenderer {
    public static void register(LevelRenderContext context) {
        // Register the rendering event
        if(!LzhClient.blocKPosToRender.isEmpty()) {
            extractAndDrawWaypoint(context);
            //renderBlockOutline(context.poseStack(), context.levelState().cameraRenderState.pos, context.consumers());
        }
    }

//    private static void renderBlockOutline(PoseStack matrices, Vec3 cameraPos, MultiBufferSource bufferSource) {
...
//        for (Wocao.PathUnit blockPos : LzhClient.blocKPosToRender) {
...
//    }
    ...
    private static final RenderPipeline FILLED_THROUGH_WALLS = RenderPipelines.register(RenderPipeline.builder(RenderPipelines.DEBUG_FILLED_SNIPPET)
            .withLocation(Identifier.fromNamespaceAndPath(Lzh.MOD_ID, "pipeline/debug_filled_box_through_walls"))
    ...
    // :::custom-pipelines:extraction-phase
    private static void renderWaypoint(LevelRenderContext context) {
        ...
        for (path.PathUnit pos:LzhClient.blocKPosToRender ){

            renderFilledBox(matrices.last().pose(), buffer, pos.targetPos.getX(), pos.targetPos.getY(), pos.targetPos.getZ(), pos.targetPos.getX() + 1f, pos.targetPos.getY()+1f, pos.targetPos.getZ() + 1f, 0f, 1f, 0f, 0.5f);
        }
    ...
    private static GpuBuffer upload(MeshData.DrawState drawParameters, VertexFormat format, MeshData builtBuffer) {
        ...
        if (vertexBuffer == null || vertexBuffer.size() < vertexBufferSize) {
            ...
            vertexBuffer = new MappableRingBuffer(() -> Lzh.MOD_ID + " example render pipeline", GpuBuffer.USAGE_VERTEX | GpuBuffer.USAGE_MAP_WRITE, vertexBufferSize);
        }
    ...
    private static void draw(Minecraft client, RenderPipeline pipeline, MeshData builtBuffer, MeshData.DrawState drawParameters, GpuBuffer vertices, VertexFormat format) {
        ...
        try (RenderPass renderPass = RenderSystem.getDevice()
                ...
                .createRenderPass(() -> Lzh.MOD_ID + " example render pipeline rendering", client.getMainRenderTarget().getColorTextureView(), OptionalInt.empty(), client.getMainRenderTarget().getDepthTextureView(), OptionalDouble.empty())) {
